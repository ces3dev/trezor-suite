...
const getBitcoinRbfParams = (
    ...
    vout.forEach(output => {
        if (!output.isAddress) {
            ...
            // blockbook sends output.hex as scriptPubKey with additional prefix where: 6a - OP_RETURN and XX - data len
            // blockbook sends ascii data in output.address field surrounded by brackets: "OP_RETURN (ASCII-VALUE)"
